...
class Model(object):
    ...
    def __init__(self,
                ...
                ent_coef,
                vf_coef,
                max_grad_norm): #Creates the step and runner models
        ...
        if max_grad_norm is not None:
            ...
            grads, grad_norm = tf.compat.v1.clip_by_global_norm(grads, max_grad_norm)
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        mb_obs, mb_actions, mb_rewards, mb_values, mb_dones = [],[],[],[],[] #lists that store the experience
...
def sf01(arr):
    s = arr.shape
    return arr.swapaxes(0, 1).reshape(s[0] * s[1], *s[2:]) #flatten the axes
   ...
   # model.load(load_path)
    ...
    for update in range(1, total_timesteps//batch_size+1):
        ...
        if update % log_interval == 0 or update == 1:
            ev = explained_variance(values, returns)#calculates the variance in y
            ...
            model.save("C:/Users/abhishek/Desktop/R.L Projects/SonicTheHedgehog-Genesis/final model/model.ckpt")
...
def play(policy, env):
    ...
    model = Model(policy=policy,
                ...
                ent_coef=0,
                vf_coef=0,
                max_grad_norm=0)#instantiate the model for step and runner
    
    # Load the model
    load_path ="C:/Users/abhishek/Desktop/R.L Projects/SonicTheHedgehog-Genesis/final model/model.ckpt"
    model.load(load_path)
